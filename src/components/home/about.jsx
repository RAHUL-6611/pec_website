import React from "react";
import pic from "../../images/OIP.jpg" 
import IT from "../../images/cse.jpg"
import ece from "../../images/cse.jpg"
import eee from "../../images/eee.jpg"
import civil from "../../images/civil.jpg"
import mectro from "../../images/mectro.jpg"
import mech from "../../images/mech.jpg"
import cse from "../../images/cse.jpg"
import hex from "../../images/hex.png"
import QuickLinks from "./quicklinks";
const About = () => {
    return <>
          <div className="about lg:flex block justify-evenly m-10">
               <div className="content ab">
                 <h1 className="font-bold">About PTU</h1>
                 <p className="pr-8">Puducherry Technological University (Erstwhile Pondicherry Engineering College) is an Institution promoted and fully funded by the Government of Puducherry, India. It was established as an Engineering College in the year 1984 under the VII Five Year Plan. The college was upgraded to technological university with the approval of the UGC with effect from 5th September, 2020. The PTU was inaugurated by Shri. Venkaiah Naidu, Hon'ble Vice President, Govt. of India on 13.09.2021. The institution is ranked 144 in Engineering Category by NIRF India Rankings 2021.</p>
                 <div className="ol flex flex-row justify-start">
                    <li>Campus</li>
                    <li>Departments</li>
                    <li>Vice Chancellor</li>
                    <li>Faculty</li>
                    <li>Students life</li>
                 </div>
               </div>
              <div className="img-slider bg-yellow-200">
                     <img src={pic} alt="" />
              </div>
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Departments</h1>
             <div className="depts flex">
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={IT} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={ece} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={eee} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>
                 <div className="cols">
                    <img src={cse} alt="" />
                    <div className="p-2">
                       <h2>IT</h2>
                       <div className="cont">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque ab non perferendis expedita nisi voluptatem ad obcaecati fugit vero ducimus?</div>
                    </div>
                 </div>

             </div>
          </div>

          <div className="recruiters m-10 ab">
             <h1 className="font-bold">Top recruiters</h1>
             <div className="rec">
                 <marquee behavior="scroll" direction="left" duration="1s">
                    <div className="companies flex">
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                      <div className="company"></div>
                    </div>
                 </marquee>
             </div>
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Partnerships</h1>
              <div className="partners flex">
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
                 <div className="partner">
                    <img src={hex} alt="" />
                 </div>
              </div>
          </div>

           <QuickLinks />

          <div className="m-10 ab">
             <h1 className="font-bold">Important events</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Student life</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Inovation center</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Student council</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">Sports</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">various clubs</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">contacts</h1>
             we can make a slider to show top recruiters
          </div>

          <div className="m-10 ab">
             <h1 className="font-bold">newsletter only for pec mail ids</h1>
             we can make a slider to show top recruiters
          </div>
          </>
}

export default About;   